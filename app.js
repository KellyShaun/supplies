...
async function loadItemsForDelivery(schoolId, companyId) {
  // Items belong to a school; prices are per school
  // company_id is stored on the delivery row, not on items
...
async function saveAllDeliveries() {
  ...
  rows.forEach(row => {
    const rowId   = row.id.replace("irow-", "");
    const itemSel = document.getElementById("irow-item-" + rowId);
    const qtyInp  = document.getElementById("irow-qty-"  + rowId);
    const unitSel = document.getElementById("irow-unit-" + rowId);
    if (!itemSel || !qtyInp) return;
    ...
    if (!itemId || !qty || qty <= 0) { valid = false; return; }
    deliveries.push({ school_id: schoolId, company_id: companyId, item_id: itemId, quantity: qty, unit, total: price * qty });
  });

  if (!valid || !deliveries.length) return showToast("Fill in all item rows with item and quantity", "error");

  const { error } = await client.from("deliveries").insert(deliveries);
  if (error) return showToast("Error saving: " + error.message, "error");

  // Clear form
  document.getElementById("deliveryItemRows").innerHTML = "";
  document.getElementById("batchTotal").textContent = "KES 0.00";
  showToast("✅ " + deliveries.length + " deliver" + (deliveries.length !== 1 ? "ies" : "y") + " saved!");
  loadDeliveries();
}
