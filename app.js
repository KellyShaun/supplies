...
function filterDeliveryItemsByCategory() {
  ...
  filtered.forEach(item=>{
    ...
    o.dataset.category=item.category||"Others";
...
// addDeliveryBatch — saves all staged rows in one batch insert
async function addDeliveryBatch(schoolId, deliveryNum, lpoNum, deliveryDate, stagingRows) {
  if (!schoolId)           return showToast("Select a school","error");
  if (!deliveryNum)        return showToast("Enter a delivery number","error");
  if (!deliveryDate)       return showToast("Enter a delivery date","error");
  if (!stagingRows.length) return showToast("Add at least one item to the list","error");

  const itemIds = [...new Set(stagingRows.map(r=>r.itemId))];
  const {data:prices,error:pErr} = await client.from("school_prices")
    .select("item_id,price").eq("school_id",schoolId).in("item_id",itemIds);
  if (pErr) return showToast("Error fetching prices: "+pErr.message,"error");

  const priceMap = {};
  (prices||[]).forEach(p=>{ priceMap[p.item_id]=Number(p.price); });

  const missing = stagingRows.filter(r=>priceMap[r.itemId]==null);
  if (missing.length) return showToast("Price not set for: "+missing.map(r=>r.itemName).join(", ")+" — set in Pricing first.","error");

  const records = stagingRows.map(r=>({
    school_id:r.schoolId, item_id:r.itemId, company_id:r.companyId||null,
    quantity:r.quantity, unit:r.unit, total:r.quantity*priceMap[r.itemId],
    delivery_number:deliveryNum, lpo_number:lpoNum||null, delivery_date:deliveryDate
  }));

  const {error} = await client.from("deliveries").insert(records);
  if (error) return showToast("Error saving: "+error.message,"error");

  showToast(records.length+" item"+(records.length!==1?"s":"")+" delivered successfully");
  loadDeliveries();
  return true;
}
